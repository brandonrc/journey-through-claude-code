...
export function NodeBox({ node, isSelected, onClick, layoutId }: Props) {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className={`relative p-6 rounded-xl cursor-pointer transition-colors ${
        node.featureGated
          ? "border border-dashed border-terminal-amber/40 bg-bg-elevated/60 opacity-80"
          : isSelected
            ? "border border-terminal-green bg-bg-surface"
            : "border border-border bg-bg-elevated hover:border-text-muted"
      }`}
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="flex gap-2">
          {node.featureGated && (
            <span className="text-xs font-mono text-terminal-amber bg-terminal-amber/10 px-2 py-0.5 rounded flex items-center gap-1">
              gated
            </span>
          )}
          {hasChildren && (
